...
class AutoUpdater:
    ...
    def download_update(self, root=None):
        try:
            ...
            update_path = os.path.join(dir_path, "update_neu.exe")
            ...
            messagebox.showinfo(
                "Update bereit",
                "Das Update wurde erfolgreich heruntergeladen!\nDas Programm wird jetzt automatisch aktualisiert."
            )
            # Starte Bootloader
            subprocess.Popen([sys.executable, BOOTLOADER_PATH, exe_path], cwd=dir_path)
            if root:
                root.destroy()
            sys.exit(0)
        except Exception as ex:
            messagebox.showerror("Update-Fehler", str(ex))
            self.close_app(root)
